    ...
    <div class="row q-px-lg">
      ...
      <div class="col">
        <p class="poppinsBold text-center" style="font-size: 0.9em">
          <q-icon class="q-pr-md" name="star_border" size="sm"/>
        ...
        <p class="poppinsRegular text-grey q-pt-md text-center"
          ...
          {{$t('seems_like_no_favorites_found')}}
        </p>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...mapActions("store",
        ["clearPublishings", "firebaseGetApprovedPublishings", "firebaseGetFavorites", "firebaseAddFavorite", "firebaseRemoveFavorite", "clearFavorites"]),
      goToPage(route) {
    ...
    computed: {
      ...mapState("store", ["publishings", "userFavoritedPublishings", "userDetails"])
    },
    created() {
    ...
    watch: {
      userDetails: function (val) {
        ...
        keys.forEach(key => {
          ...
          item.key = key
          let userFavoritedPublishingsKeys = Object.keys(this.userFavoritedPublishings);
          userFavoritedPublishingsKeys.forEach(userFavoritedPublishingsKey => {
            let item2 = this.userFavoritedPublishings[userFavoritedPublishingsKey];
      ...
      userFavoritedPublishings: function (val) {
        this.firebaseGetFavorites();
        this.orderedPublishings = [];
        let keys = Object.keys(this.publishings);
        keys.forEach(key => {
